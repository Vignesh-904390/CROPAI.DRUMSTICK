...
MODEL_PATH = 'model/drumstick_effnet.pth'
DISEASE_JSON_PATH = 'drumstick_disease_info.json'
...
def send_daily_report():
    if daily_stats["count"]==0: return
    try:
        msg=Message("📊 Daily Click Report - Drumstick Disease Detection",
...
def predict_image():
    ...
    if 'image' not in request.files or request.files['image'].filename=='':
        flash("❌ No image uploaded","danger"); return redirect('/')
...
def predict_video():
    ...
    if 'video' not in request.files or request.files['video'].filename=='':
        flash("❌ No video uploaded","danger"); return redirect('/')
    f=request.files['video']
    name=secure_filename(f.filename)
    vp=os.path.join(UPLOAD_FOLDER,name); f.save(vp)
    ...
    if not preds:
        flash("⚠️ No disease found","warning"); return redirect('/')

    c=Counter(preds)
    mc=[l for l,n in c.items() if n>=2] or list(c.keys())
    if any(l.lower()=="healthy" for l in mc): mc=["Healthy"]
...
def send_email():
    log_click()
    name=request.form.get('name'); email=request.form.get('email'); msgt=request.form.get('message')
    photo=request.files.get('photo')
    if not(name and email and msgt):
        flash("❗ Fill all fields","warning"); return redirect('/')
    try:
