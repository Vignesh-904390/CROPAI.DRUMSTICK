...
model.to(device)
model.eval()
print("🧠 Ladies Finger EfficientNet model loaded.")
...
def send_daily_report():
    if daily_stats["count"] == 0:
        return
    try:
        msg = Message("📊 Daily Click Report - Ladies Finger Disease Detection",
...
def send_prediction_result_email(filename, prediction_results, image_path=None):
    try:
        msg = Message("🌱 New Ladies Finger Disease Detection Result",
                      sender=app.config['MAIL_USERNAME'],
                      recipients=['tdaitech@gmail.com'])
        
        # Create email body with prediction results
        email_body = f"""
        🔍 Ladies Finger Disease Detection Result
